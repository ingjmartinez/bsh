...
class AgenciaDeltaController extends Controller
{
    ...
    {
        return view('agencias-delta.index');
    }
    ...
    public function create()
    {
        ...
        return view('agencias-delta.create', compact('operadores', 'coordinadores'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('agencias-delta.index')
            ->with('success', 'Agencia Delta creada exitosamente.');
    }
    ...
    public function update(Request $request, AgenciaLotedom $agenciaLotedom)
    {
        ...
        return redirect()->route('agencias-delta.index')
            ->with('success', 'Agencia Delta actualizada exitosamente.');
    }
    ...
    public function list(Request $request)
    {
        ...
        // Total de registros
        $totalRecords = $this->queryAgenciasDelta()->count();
        ...
        $totalDelta = $this->queryAgenciasDelta()->count();
    ...
    {
        $resultado = $this->obtenerTerminalesNoRegistradasDesdeVentasDelta();
    ...
    public function sincronizarNoRegistradasDesdeVentasUsuarios(Request $request)
    {
        ...
        try {
            ...
            Log::error('Error sincronizando agencias delta desde ventas guardadas.', [
                'error' => $e->getMessage(),
                'line' => $e->getLine(),
                'file' => $e->getFile(),
            ]);
            ...
            return response()->json([
                'ok' => false,
                'message' => 'No se pudieron sincronizar las agencias no registradas desde ventas Delta.',
            ], 500);
        }
    ...
    public function agenciasParaActualizar(Request $request)
    {
        ...
        $agencias = $this->queryAgenciasDelta()
            ...
            ->map(function (AgenciaLotedom $agencia) use ($campos) {
                ...
                return [
                    ...
                    'edit_url' => route('agencias-delta.edit', $agencia),
                ];
            })
            ->filter()
            ->values();
    ...
    public function actualizarEstatusAgencia(Request $request)
    {
        ...
        $agencia = $this->queryAgenciasDelta()->findOrFail((int) $validated['agencia_id']);
    ...
    {
        return $this->queryAgenciasDelta()
            ->selectRaw('id, COALESCE(codigo, agencia) AS agencia, terminal, COALESCE(nombre, nombre_agencia) AS nombre_agencia, empresa, ciudad, ruta, estatus')
            ->where('estatus', 0)
            ->orderBy('terminal')
            ->get()
    ...
    private function obtenerAgenciasNoRegistradasConVentaTreintaDias(): array
    {
        ...
        $ventas = DB::table(self::FUENTE_VENTAS_USUARIOS)
            ...
            ->whereRaw('(COALESCE(venta_loteria, 0) + COALESCE(venta_recarga, 0) + COALESCE(ventas_no_tradicional, 0)) > 0');
    ...
    {
        return view('agencias-delta.incumplimientos');
    }
    ...
    private function obtenerTerminalesNoRegistradasDesdeVentasDelta(): array
    {
        $terminalesRegistradas = $this->queryAgenciasDelta()
            ->whereNotNull('terminal')
            ->pluck('terminal')
    ...
    private function buscarAgenciaParaActualizacion(array $row): ?AgenciaLotedom
    {
        ...
        if ($id !== null && $id !== '') {
            return $this->queryAgenciasDelta()->find((int) $id);
        }

        $terminal = $this->valorColumna($row, ['terminal']);
        if ($terminal !== null && trim((string) $terminal) !== '') {
            return $this->queryAgenciasDelta()->where('terminal', trim((string) $terminal))->first();
        }

        $codigoAgencia = $this->valorColumna($row, ['agencia']);
        if ($codigoAgencia !== null && trim((string) $codigoAgencia) !== '') {
            return $this->queryAgenciasDelta()->where('agencia', trim((string) $codigoAgencia))->first();
        }

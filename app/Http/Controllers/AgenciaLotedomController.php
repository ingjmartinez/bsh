...
use App\Imports\AgenciasLotedomImport;
...
class AgenciaLotedomController extends Controller
{
    private const FUENTE_VENTAS_USUARIOS = 'ventas_usuarios_net';
    ...
    {
        return view('agencias-lotedom.index');
    }
    ...
    public function create()
    {
        ...
        return view('agencias-lotedom.create', compact('operadores', 'coordinadores'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'sistema' => 'nullable|string|max:55',
        ...
        $validated['codigo'] = $validated['agencia'];
        $validated['nombre'] = $validated['nombre_agencia'] ?? null;
        $validated['sistema'] = trim((string) ($validated['sistema'] ?? '')) ?: 'lotedom';
        ...
        return redirect()->route('agencias-lotedom.index')
            ->with('success', 'Agencia Lotedom creada exitosamente.');
    }
    ...
    {
        return view('agencias-lotedom.show', ['agencia' => $agenciaLotedom]);
    }
    ...
    public function edit(AgenciaLotedom $agenciaLotedom)
    {
        ...
        return view('agencias-lotedom.edit', [
    ...
    public function update(Request $request, AgenciaLotedom $agenciaLotedom)
    {
        ...
        return redirect()->route('agencias-lotedom.index')
            ->with('success', 'Agencia Lotedom actualizada exitosamente.');
    }
    ...
    {
        // Las tablas pivote actuales referencian exclusivamente `agencias`.
        // En el modulo Lotedom conservamos los campos operador/coordinador
        // en la propia fila para evitar cruces de llaves foraneas con el catalogo principal.
    }
    ...
    public function destroy(AgenciaLotedom $agenciaLotedom)
    {
        ...
        return redirect()->route('agencias-lotedom.index')
            ->with('success', 'Agencia Lotedom eliminada exitosamente.');
    }
    ...
    public function list(Request $request)
    {
        ...
        $empresaFilter = $request->input('empresa_filter', 'lotedom');
        ...
        if ($empresaFilter === 'lotedom') {
            $query->where(function ($empresaQuery) {
                $empresaQuery
                    ->whereRaw('LOWER(COALESCE(sistema, "")) LIKE ?', ['%lotedom%'])
                    ->orWhereRaw('LOWER(COALESCE(empresa, "")) LIKE ?', ['%lotedom%']);
            });
        }
        ...
        // Total de registros
        $totalRecords = AgenciaLotedom::count();
        ...
        $totalActivas = AgenciaLotedom::query()->where('estatus', 1)->count();
        $totalInactivas = AgenciaLotedom::query()->where('estatus', 0)->count();
        $totalLotedom = AgenciaLotedom::query()
    ...
    public function noRegistradasDesdeVentasUsuarios(Request $request)
    {
        $resultado = $this->obtenerTerminalesNoRegistradasDesdeVentasUsuariosLotedom();
    ...
    public function sincronizarNoRegistradasDesdeVentasUsuarios(Request $request)
    {
        $resultado = $this->obtenerTerminalesNoRegistradasDesdeVentasUsuariosLotedom();
        ...
        try {
            ...
            Log::error('Error sincronizando agencias lotedom desde ventas por usuario.', [
                'error' => $e->getMessage(),
                'line' => $e->getLine(),
                'file' => $e->getFile(),
            ]);
            ...
            return response()->json([
                'ok' => false,
                'message' => 'No se pudieron sincronizar las agencias no registradas desde ventas por usuario.',
            ], 500);
        }
        ...
        $agencias = AgenciaLotedom::query()
            ...
            ->map(function (AgenciaLotedom $agencia) use ($campos) {
                ...
                return [
                    ...
                    'edit_url' => route('agencias-lotedom.edit', $agencia),
                ];
            })
            ->filter()
            ->values();
    ...
    public function actualizarEstatusAgencia(Request $request)
    {
        ...
        $agencia = AgenciaLotedom::findOrFail((int) $validated['agencia_id']);
    ...
    private function registrarTerminalesBase(array $terminales): array
    {
        ...
        $terminalesExistentes = AgenciaLotedom::query()
    ...
    private function obtenerAgenciasSinVentaTreintaDias(): array
    {
        ...
        $agencias = AgenciaLotedom::query()
            ->selectRaw('id, COALESCE(codigo, agencia) AS agencia, terminal, COALESCE(nombre, nombre_agencia) AS nombre_agencia, empresa, ciudad, ruta, estatus')
            ->whereNotNull('terminal')
            ->where('estatus', 1)
            ->orderBy('terminal')
            ->get()
    ...
    private function obtenerAgenciasNoRegistradasConVentaTreintaDias(): array
    {
        ...
        $terminalesRegistradas = AgenciaLotedom::query()
            ->whereNotNull('terminal')
            ->pluck('terminal')
            ->map(fn($terminal) => $this->normalizarTerminal((string) $terminal))
            ->filter(fn($terminal) => $terminal !== '0')
    ...
    {
        return view('agencias-lotedom.incumplimientos');
    }
    ...
    public function listIncumplimientosHorario(Request $request)
    {
        ...
        $agencias = AgenciaLotedom::query()
            ->selectRaw('id, COALESCE(codigo, agencia) AS agencia, COALESCE(nombre, nombre_agencia) AS nombre_agencia, terminal, horario_am, horario_pm')
            ->whereNotNull('terminal')
            ->where(function ($q) {
    ...
    private function obtenerTerminalesNoRegistradasDesdeVentasUsuariosLotedom(): array
    {
        $terminalesRegistradas = AgenciaLotedom::query()
            ->whereNotNull('terminal')
            ->pluck('terminal')
        ...
        $ventas = DB::table(self::FUENTE_VENTAS_USUARIOS)
            ->select('agencia_id', 'monto', 'fecha')
            ->whereNotNull('agencia_id')
            ->whereRaw('COALESCE(monto, 0) > 0');

        $terminalesNoRegistradas = $ventas
            ->get()
            ->map(function ($row) {
                $terminalOriginal = trim((string) ($row->agencia_id ?? ''));
                $terminalKey = $this->normalizarTerminal($terminalOriginal);
                ...
                return [
                    ...
                    'fecha' => $row->fecha ? Carbon::parse((string) $row->fecha)->toDateString() : null,
                ];
            })
            ...
            ->map(function ($rows, $terminalKey) {
                ...
                $terminalOriginal = (string) ($rows->pluck('terminal_original')->filter()->first() ?? '');
                $fechas = $rows->pluck('fecha')->filter()->unique()->values();

                return [
                    'terminal' => $terminalOriginal !== '' ? $terminalOriginal : (string) $terminalKey,
                    'terminal_key' => (string) $terminalKey,
                    'dias_con_venta' => $fechas->count(),
                    'primera_fecha' => $fechas->min(),
                    'ultima_fecha' => $fechas->max(),
                    'total_venta' => round((float) $rows->sum('monto'), 2),
                ];
            })
            ->sortByDesc('total_venta')
            ->values()
            ->all();

        $desde = collect($terminalesNoRegistradas)->pluck('primera_fecha')->filter()->min();
        $hasta = collect($terminalesNoRegistradas)->pluck('ultima_fecha')->filter()->max();
    ...
    {
        return Excel::download(new AgenciasLotedomExport, 'agencias_lotedom_' . date('Y-m-d_His') . '.xlsx');
    }
    ...
    public function import(Request $request)
    {
        ...
        try {
            $import = new AgenciasLotedomImport();
    ...
    public function massUpdatePreview(Request $request)
    {
        ...
        try {
            ...
            $terminalesEncontradas = AgenciaLotedom::query()
    ...
    private function buscarAgenciaParaActualizacion(array $row): ?AgenciaLotedom
    {
        ...
        if ($id !== null && $id !== '') {
            return AgenciaLotedom::query()->find((int) $id);
        }

        $terminal = $this->valorColumna($row, ['terminal']);
        if ($terminal !== null && trim((string) $terminal) !== '') {
            return AgenciaLotedom::query()->where('terminal', trim((string) $terminal))->first();
        }

        $codigoAgencia = $this->valorColumna($row, ['agencia']);
        if ($codigoAgencia !== null && trim((string) $codigoAgencia) !== '') {
            return AgenciaLotedom::query()->where('agencia', trim((string) $codigoAgencia))->first();
        }
    ...
    public function template()
    {
        ...
        $data = [
            $headers,
            ['20907', '5546', '7:00 AM / 2:00 PM', '2:00 PM / 9:00 PM', 'Agencia Lotedom Ejemplo', 'Lotedom', 'Grupo A', 'San Pedro', 'Ruta 0501', 'Jose Ruby', 'Aramis', '1', 'SI'],
        ];

        $filename = 'plantilla_agencias_lotedom.xlsx';
    ...
    public function massUpdateTemplate()
    {
        ...
        $filename = 'plantilla_actualizacion_masiva_agencias_lotedom.xlsx';

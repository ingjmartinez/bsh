                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0">Ventas Delta</h4>
...
@section('script')
    <script>
        ...
        btnGenerarData.addEventListener('click', () => {
            ...
            fetch(`/get-ventas_delta?fecha=${fecha}`)
                .then(response => response.json())
                .then(data => {
                    if (data.code != 0) {
                        Swal.fire({
        ...
        btnGuardarData.addEventListener('click', () => {
            ...
            fetch(`/save-ventas_delta?fecha=${fecha}`, {
                    method: 'POST',
                    headers: {
        ...
        btnEliminarData.addEventListener('click', () => {
            ...
            fetch(`/delete-ventas_delta?fecha=${fecha}`)
                .then(response => response.json())
                .then(data => {
                    Swal.fire({
                        title: "Listo",
        ...
        btnGuardarDataFecha.addEventListener('click', async () => {
            ...
            try {
                ...
                for (let i = 0; i < dates.length; i++) {
                    ...
                    const response = await fetch(`/save-ventas_delta?fecha=${date}`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        btnEliminarDataFecha.addEventListener('click', async () => {
            ...
            try {
                ...
                for (let i = 0; i < dates.length; i++) {
                    ...
                    const response = await fetch(`/delete-ventas_delta?fecha=${date}`);
                    if (!response.ok) {
                        const text = await response.text().catch(() => null);
                        throw new Error(text || `Error HTTP ${response.status}`);
                    }
